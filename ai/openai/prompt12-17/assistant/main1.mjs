...
const main = async () => {
    ...
    // `//有这种情况-->AI客服联系客户-->解决问题(删除评论)
    // const prompt = `
    // 你是一名客服AI助理。
    // 你的任务是向尊贵的客户发送电子邮件回复。
    // 给定以'''分隔的评论的客户发送电子邮件，
    // 生成回复已感谢客户的评价。
    // 如果情绪是正面或者中性的，给那些他们的评价
  ...
  `
  //吴恩达 prompt 系列 machine learing  机器学习

  //nlp
//   const prompt  = `
//   以下产品评论的情感是什么？
//   以三个反引号分隔
//   请用一个词来回答，可以是"消极"、"积极"、"中性"
//   评论内容：'''${lamp_review}'''
//   `
// const prompt  = `
//    以下产品评论的情感是什么？
...
// 评论以三个反引号分隔。
// 将你的回复格式化为JSON对象，其中"商品"和"品牌"作为建。
// 如果信息不存在，请使用"未知"作为值。
// 让您的回复尽可能简短。
